...
export default function ReleaseToggle({ subtopicId, initiallyReleased, orgId }: ReleaseToggleProps) {
  ...
  const handleSaved = async () => {
    ...
    try {
      const res = await fetch(`/api/release-subtopic?contentId=${subtopicId}`)
      if (res.ok) {
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {released && orgId && (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md border border-gray-200 px-2 py-1.5 text-xs text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors"
              title="Manage which students can see this subtopic"
            >
      ...
      {showStudentModal && (
        <StudentReleaseModal
          apiEndpoint="/api/release-subtopic"
